...
export function validateLeticiaDemoScenario(): string[] {
  ...
  const { distribution, qualifiedMentions, netSentiment } = LETICIA_DEMO_SCENARIO.overview;
  const sentimentTotal = Object.values(distribution).reduce((total, count) => total + count, 0);
  const expectedNetSentiment = sentimentTotal === 0
    ? 0
    : ((distribution.positif - distribution.negatif) / sentimentTotal) * 100;
  if (netSentiment === null || Math.abs(netSentiment - expectedNetSentiment) > Number.EPSILON) errors.push("Overview net sentiment must match its sentiment distribution.");
  if (LETICIA_DEMO_SCENARIO.overview.insufficientSample !== (qualifiedMentions < 30)) errors.push("Overview insufficient sample flag must match the Dashboard threshold.");
