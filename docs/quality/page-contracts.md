...
- Visible actions: type a query, run search, toggle source chips, paginate verbatims, expand a row to inspect the full verbatim.
...
- Current status: partially functional; search, source filtering, and pagination work against the API.
- Decorative or dead controls: the `Filtrer` chip is decorative; the `open_in_new` icon on search results has no handler; the floating chat button is decorative.

## Watchlists
- Route: `/watchlists` (`#/watchlists`)
- Purpose: create watchlists, inspect their current metrics, and deactivate active ones.
- Visible actions: switch between all/active/inactive, open the create form, create a watchlist, select a watchlist, deactivate an active watchlist, inspect metrics and quick insight.
...
- Decorative or dead controls: the `headerSearchPlaceholder` is inert because `onSearch` is a no-op; `Voir les détails analytiques` is decorative.

## Alertes
- Route: `/alertes` (`#/alertes`)
- Purpose: triage alerts and move them through the status lifecycle.
- Visible actions: filter by status, filter by severity, select an alert, acknowledge, dismiss, or resolve it.
...
- Decorative or dead controls: the app-shell search field is inert on this page.
...
- Visible actions: choose trigger/provider/model, generate recommendations, archive all active recommendations, archive or dismiss one recommendation, inspect run history.
...
- Decorative or dead controls: the floating `auto_awesome` button is decorative; the kebab icon on each recommendation card is decorative.

## Campagnes
- Route: `/campagnes` (`#/campagnes`)
- Purpose: create campaigns, browse the campaign list, and inspect NSS impact by campaign.
- Visible actions: create a campaign, filter the list by status, select a campaign row, page through the list, inspect the impact panel for the selected campaign.
...
- Decorative or dead controls: `EXPORTER DATA` and the top-right `CRÉER UNE CAMPAGNE` button are shell-only; they do not trigger actions in the current code.
...
- Decorative or dead controls: top-nav `Ingestion`, `Pipelines`, and `Logs` items are decorative; `notifications`, `settings`, `New Pipeline`, `Support`, and `Documentation` are shell-only.
