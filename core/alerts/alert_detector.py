...
def run_alert_detection(df_annotated: pd.DataFrame) -> list[str]:
    ...
    for watchlist in list_watchlists():
        ...
        if current_nss is not None and current_nss < 20:
            alert_id = _create_detection_alert(
                watchlist,
                "nss_critical_low",
                title=f"NSS critique sur {watchlist['watchlist_name']}",
                description=(
                    f"Le NSS courant est a {current_nss:.1f}, sous le seuil de 20 sur la watchlist "
        ...
        if metrics["volume_current"] > 0 and negative_ratio > 60:
            alert_id = _create_detection_alert(
                watchlist,
        ...
        if pd.isna(last_signal_at) or last_signal_at <= reference_time - pd.Timedelta(days=7):
            alert_id = _create_detection_alert(
                watchlist,
                "no_recent_signals",
                title=f"Aucun signal recent pour {watchlist['watchlist_name']}",
                description=(
                    f"Aucun signal n'a ete detecte sur les 7 derniers jours pour "
                    f"{watchlist['watchlist_name']}."
                ),
                metrics=metrics,
                metric_current=0,
        ...
        if metrics["volume_previous"] > 0 and metrics["volume_current"] < (metrics["volume_previous"] * 0.5):
            ...
            alert_id = _create_detection_alert(
                ...
                description=(
                    f"Le volume courant ({metrics['volume_current']}) est inferieur a 50% du volume "
                    f"de la periode precedente ({metrics['volume_previous']})."
                ),
                metrics=metrics,
